...
| `assets.py` | 素材层：CDN 图/本地立绘/UI 图标 → data: URL，内存+落盘缓存与字节预算、`_tower_icon`/`_boss_event_asset` 等映射 |
| `collect.py` | 采集层：`collect_overview/daily/rules/maps/odyssey/player/rush/leaderboard(_page)`、`fetch_leaderboard_page`、`fetch_rank_entry`、`_safe` |
| `textfmt.py` | 文本渲染：`build_overview`、`_single_event_text`、`format_rules`、排行/地图/远征/玩家/rush 文本 |
| `cards/` | 渲染层：`common`(四种外壳 CSS)/`overview`/`leaderboard`/`odyssey`/`rules`/`rush`/`player`/`help`；`__init__` 为渲染管线（`_render_card`/`_send_card`/`_finish_multi_cards`）并统一再导出 |
| `push.py` | 后台任务：history.json 归档、`_prewarm_once` 预热、活动刷新推送（`_btd6_push_*`、`_set_last_pushed`、`_push_change_group`）与全部 apscheduler 定时任务 |
| `handlers.py` | 17 个 nonebot matcher/命令 handler 与参数解析（`parse_kind` 等）；命令参数词表在此 |
