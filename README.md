...
**抽签运势 · 守望先锋战绩 · OW补丁推送 · 群聊统计 · 每日晨报 · 节假日倒计时 · AI 聊天 · 申请管理**
...
| 🎮 守望先锋 | 战报 / 段位 / 强度 / 总结 | 国服数据（经查询机器人任务中继，串行队列），绑定一次即可查询；查询冷却；绘制超过 QQ 5 分钟时限时自动冻结并约 5 分钟后领取图片转发 |
| 🛡️ OW补丁推送 | 国服补丁说明长截图 | 每小时轮询官网补丁页，新补丁自动发全文长截图（失败重试 3 次→分块→标题卡三级回退，图片过大/过多自动降级）；按群订阅，仅主人可开关 |
| 🕹 上号组队榜 | 开黑小队组队 | `.上号A=B` 开队（A=已有人数含自己、B=还缺，如 `.上号2=3` 开 5 人小队并当队长）、`.加入N` 入队、`.谁玩` 按队号列出小队（显示 OW 绑定名/群名片，不 @ 人）、`.下班` 队长解散全队 / 队员退出；**带点严格完全匹配**才触发（缺点/@/引用/图片混排不响应）；小队 24 小时自动解散，按群开关需主人 `.上号开启` |
...
# 可选：仅在测试环境启用「特权群 owner」——默认空，生产请勿配置
# QQBOT_TEST_PRIVILEGED_GROUPS=群号1,群号2
...
| 命令 | 说明 |
|------|------|
| 命令「.上号A=B」「.加入N」「.谁玩」「.下班」 | 上号组队榜（需主人先在群内 `.上号开启`）：开队（A=已有含自己、B=还缺）/ 入队 / 查小队名单（OW 绑定名优先，未绑定用群名片）/ 队长解散、队员退出；带点严格完全匹配才触发（缺点/@/引用/图片混排不响应） |
...
| 🛡️ OW补丁 | `.ow补丁订阅/退订/状态/测试`（按群订阅，每小时检查，新补丁发全文长截图） |
| ⏳ 倒计时 | `.倒计时开启/关闭/状态/测试` |
...
| 🕹 上号榜 | `.上号开启 [群号]` `.上号关闭 [群号]` `.上号状态`（开启后群里组队命令才生效） |
...
plugins/
  ...
  ├── news/                 每日晨报
  ├── on_duty/              上号组队榜（.上号A=B 开队 / .加入N / .谁玩 / .下班，24h 自动解散，按群开关）
  ├── ow_patch/             OW国服补丁推送（每小时轮询官网补丁页，新补丁全文长截图发订阅群）
...
测试使用内置 stub 模拟 NoneBot 环境（当前约 458 项），无需安装真实 NoneBot、数据库或 NapCat。推送到 GitHub 后 Actions 会在 Python 3.10/3.12 的干净环境自动安装依赖并跑测试 + ruff 检查（见 `.github/workflows/ci.yml`）。
...
- 本仓库**仅包含代码**，不含任何运行配置
- `.gitignore` 已排除全部敏感文件：`.env`、`db.json`、`config.json`、`ai_config.json`、`bindings.json`、各类 `state.json`（含 `verify_state.json`）等（含 QQ 号 / 群号 / 密钥 / 聊天记录）
