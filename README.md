...
| 💸 价格对比 | 大模型价格图 | 主流大模型 API 价格实时对比（OpenRouter） |
| ⏳ 倒计时 | 周末 / 节假日 | 图片卡片 + 每日 17:00 群推送 |
| 💬 AI 聊天 | @机器人 对话 / 戳一戳 | 西野七濑人设（[人设表](docs/西野七濑人设表.md)）+ 多用户记忆 + 戳一戳 AI 上下文回复 |
| 🔑 关键词触发 | 群内自动回复 | 关键词 → 自定义回复，60 秒冷却 |
| 🛡 申请管理 | 好友 / 进群申请 | 关键字自动通过（不区分大小写）+ 私聊审批 |
...
| `.我的ID` / `.解绑` | 查看 / 解除绑定 |
| `.倒计时` | 下一个周末和节假日倒计时 |
| `.ai` | 主流大模型 API 价格对比图（实时抓取，失败回退缓存；别名 `.价格图`） |
| `@机器人 + 消息` | 与 AI 角色聊天 |
...
| 类别 | 命令 |
|------|------|
| 📊 统计 | `.龙王` `.词云 [N]` `.统计开启/关闭/状态` |
| 📰 推送 | `.新闻开启/关闭/测试/状态` `.词云开启/关闭/状态` `.倒计时开启/关闭/状态/测试` |
| 💸 价格 | `.ai 更新`（强制刷新价格缓存） |
| 🔑 关键词 | `.add 关键词 [回复]` `.关键词删除` `.关键词列表` |
...
plugins/
  ...
  ├── keyword_reply/        关键词触发回复
  ├── llm_price/            大模型价格对比图（OpenRouter 实时 + 本地缓存回退）
  ├── news/                 每日新闻推送
  ├── owstats/              守望先锋战绩查询
  ├── repeater/             自动复读
