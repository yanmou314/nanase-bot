...
| 💬 AI 聊天 | @机器人 对话 / 戳一戳 | 西野七濑人设（[人设表](docs/西野七濑人设表.md)）+ 多用户记忆；群聊携带发言人昵称可分辨多人；双层日配额熔断（全局 500 / 单用户 50，可调，重启不清零） |
| 🔀 随机插话 | 群聊随机回复 | 概率可调、每群 60 秒最小间隔，复用 AI 聊天接口，独立日限额（默认 80 条/天）不计入 @ 对话额度 |
...
| 🚨 报错通知 | 运行异常监控 | 插件处理报错 / 定时任务失败 / 被插件捕获后仅落 WARNING+ 日志的异常，自动私聊通知主人（含来源、错误、位置，同类错误 10 分钟冷却；框架噪音与设计内可选失败豁免） |
| 🔁 自动复读 | 群消息复读 | 去重 + 冷却 + 状态持久化（只存消息哈希） |
| 📈 指令统计 | 每日使用报告 | 每日 00:05 定时生成统计图推送 |
| 🚪 进出群播报 | 欢迎 / 退群通知 | 入群欢迎、退群随机文案并播报逗留时长 |
| 🧹 潜水清人 | 按最后发言时间清理 | 主人按群配置清人时间与保护名单，预览确认后执行（回执经纯文本段包裹防 CQ 注入），每轮上限与失败记账，机器人非管理员自动拦截 |
...
# 可选：AI 聊天日配额熔断（默认 500/全局、50/单用户；<=0 关闭）
# QQBOT_AI_DAILY_LIMIT=500
# QQBOT_AI_USER_DAILY_LIMIT=50
# 可选：随机插话独立日限额（默认 80；<=0 关闭）
# QQBOT_RANDOM_CHAT_DAILY_LIMIT=80
...
| `.btd6排行 竞赛\|boss\|领土 [P页码\|排名]` | BTD6 排行榜：默认前25；`P2`=第2页；数字=该名次玩家档案（boss 自动返回标准+精英双榜，领土 自动返回个人+战队双榜） |
...
plugins/
  ...
  ├── idle_kick/            潜水清人（按最后发言时间，预览确认后执行，保护名单/每轮上限）
  ├── news/                 每日晨报
...
测试使用内置 stub 模拟 NoneBot 环境（当前约 489 项），无需安装真实 NoneBot、数据库或 NapCat。推送到 GitHub 后 Actions 会在 Python 3.10/3.12 的干净环境自动安装依赖并跑测试 + ruff 检查（见 `.github/workflows/ci.yml`）。
...
- 状态文件损坏时自动备份为 `.corrupt-*` 再重建，不会静默清空配置；原子写入沿用原文件权限并统一收紧到 600（状态文件普遍含 QQ 号/密钥），目录 fsync 保证掉电不回退旧版
- 「测试特权群」必须同时配置 UID 白名单，否则 bot 拒绝启动（防止整群成员获得管理权限）
